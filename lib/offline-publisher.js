...
async function publishPackages(config) {
  ...
  // Start completed counter from pre-skipped count so progress shows correctly
  let completed = skipped;

  // Show initial progress if we have pre-skipped packages
  if (skipped > 0) {
    ...
    limit(async () => {
      ...
      if (skipExisting && cached) {
        ...
        if (preCheck?.exists && preCheck?.certain) {
          // Already counted in skipped and completed at initialization
          return {
            status: STATUS.SKIPPED,
            package: packageId,
