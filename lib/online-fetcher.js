...
async function fetchDependencies(config) {
  ...
  // Pre-check existing files to initialize counter correctly
  const preCheckSpinner = ora('Checking existing files...').start();
  let preExistingCount = 0;
  const packagesArray = Array.from(packages.values());

  for (const packageInfo of packagesArray) {
    const { name, version } = packageInfo;
    const fileName = `${name.replace('/', '-').replace('@', '')}-${version}.tgz`;
    const filePath = path.join(outputDir, fileName);
    if (await fs.pathExists(filePath)) {
  ...
  if (preExistingCount > 0) {
    preCheckSpinner.succeed(`Found ${preExistingCount} packages already downloaded`);
  } else {
    preCheckSpinner.succeed('No existing packages found');
  }
  ...
  let skipped = 0;
  const errors = [];
  // Start completed counter from pre-existing count so progress shows correctly
  let completed = preExistingCount;

  // Show initial progress if we have pre-existing packages
  if (preExistingCount > 0) {
    downloadSpinner.text = `Downloading... ✅ ${downloaded} | ⏭️  ${preExistingCount} | ❌ ${errors.length} (${completed}/${packages.size})`;
  }

  const downloadPromises = packagesArray.map(packageInfo =>
    limit(async () => {
      ...
      if (result.status === STATUS.SUCCESS) {
        ...
        // Already counted in preExistingCount, just update skipped display
        skipped++;
      } else {
        completed++;
        errors.push(result);
